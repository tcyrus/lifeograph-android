...
public class ChartPoints {
    ...
    int calculate_distance(LDate d1, LDate d2) {
    ...
    int calculate_distance_neg(LDate d1, LDate d2) {
        switch (type & PERIOD_MASK) {
            case MONTHLY:
                return LDate.calculate_months_between_neg(d1.m_date, d2.m_date);
            case YEARLY:
    ...
    void addPlain(LDate d_last, LDate d) {
        if (d.is_ordinal())
            return;
